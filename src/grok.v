...
pub fn run() {
	mut app := &App{}
	args := os.args
	base_dir := if args.len > 1 && args[1].len > 0 {
		args[1]
	} else if pic_dir := os.getenv('PIC_DIR'); pic_dir.len > 0 {
		pic_dir
	} else {
		'.'
	}

	files := os.ls(base_dir) or { panic('Failed to list files in ${base_dir}') }
	app.img_paths = files.filter(fn (f string) bool {
...
fn (mut app App) next_img() {
	if app.img_paths.len == 0 {
		return
	}
	start := app.img_index
	for _ in 0 .. app.img_paths.len {
		start = (start + 1) % app.img_paths.len
		if app.load_img_at(start) {
			app.view_mode = .fit
			app.fit_to_window()
			return
		}
	}
	eprintln('No loadable next image found.')
}

fn (mut app App) prev_img() {
	if app.img_paths.len == 0 {
		return
	}
	start := app.img_index
...
fn (mut app App) prev_img() {
	...
	start := app.img_index
	for _ in 0 .. app.img_paths.len {
		start = (start - 1 + app.img_paths.len) % app.img_paths.len
		if app.load_img_at(start) {
			app.view_mode = .fit
			app.fit_to_window()
			return
	...
	eprintln('No loadable previous image found.')
}
...
fn (mut app App) status_text() string {
	...
	return 'pic: ${app.img_paths[app.img_index]}  size: ${app.img.width}x${app.img.height}  index: ${app.img_index + 1}/${app.img_paths.len}  scale: ${app.scale:.2f}  mode: ${view_mode_str}  fullscreen: ${if is_fullscreen { "Y" } else { "N" }}  slide: ${if app.slide_mode { "Y" } else { "N" }}(${app.slide_interval}s)'
}
